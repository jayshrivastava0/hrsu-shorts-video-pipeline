...
Drop .mp4/.mov clips into asset_library/footage/ and describe each one in
asset_library/footage/manifest.json. This module scores each manifest entry
...
FOOTAGE_DIR = Path("asset_library/footage")
...
def _load_manifest() -> list[dict]:
    ...
    try:
        ...
        log.warning("Footage manifest malformed (%s); ignoring", e)
        return []
    ...
    for entry in raw:
        ...
        path = FOOTAGE_DIR / fn
        if not path.exists():
            log.warning("Footage manifest references missing file: %s", path)
            continue
...
def _score(entry: dict, scene_tokens: set[str], category: str | None,
    ...
    if category and category.lower() in entry_cats:
        score += 5
    # Tag overlap with scene narration / on-screen text.
    score += 2 * len(scene_tokens & entry_tags)
    # Description-token overlap (weaker).
    score += len(scene_tokens & entry_desc_tokens)
    # Bonus for clips flagged as suitable for hooks when visual_type=hrsu_edge,
    # or for broll when visual_type=stock.
...
def find_footage(scene: dict) -> dict | None:
    ...
    best = scored[0][2]
    log.info("Footage match for scene %s -> %s (score %d)",
