...
from video_agent.visual_engine import footage_library as fl


def _setup(tmp_path, monkeypatch, manifest, files):
    fdir = tmp_path / "footage"
    fdir.mkdir()
    for name in files:
        (fdir / name).write_bytes(b"\x00\x00\x00\x18ftypmp42")
    (fdir / "manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
    monkeypatch.setattr(fl, "FOOTAGE_DIR", fdir)
    monkeypatch.setattr(fl, "MANIFEST_PATH", fdir / "manifest.json")
    fl.reset_cache()


def test_returns_none_when_no_manifest(tmp_path, monkeypatch):
    monkeypatch.setattr(fl, "FOOTAGE_DIR", tmp_path / "empty")
    monkeypatch.setattr(fl, "MANIFEST_PATH", tmp_path / "empty" / "manifest.json")
    fl.reset_cache()
    assert fl.find_footage({"narration": "anything"}) is None
...
def test_returns_none_below_min_score(tmp_path, monkeypatch):
    ...
    assert fl.find_footage(scene) is None
...
def test_skips_manifest_entries_with_missing_files(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fl, "MANIFEST_PATH", fdir / "manifest.json")
    fl.reset_cache()
    assert fl.find_footage({"narration": "wastewater",
                            "category": "wastewater_treatment",
                            "visual_spec": {}}) is None
...
def test_deterministic_tiebreak_by_id(tmp_path, monkeypatch):
    ...
    # With identical scores, alphabetical id wins → "a_clip".
    match = fl.find_footage(scene)
